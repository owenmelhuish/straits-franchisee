...
export default async function HistoryPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="mb-6 text-[16px] font-semibold text-[#1A1A1A]">{t.dashboardHistory.title}</h1>

      {submissions.length === 0 ? (
        <div className="rounded-3xl bg-white p-12 text-center shadow-[0px_4px_20px_rgba(0,0,0,0.04)]">
          <p className="text-[#666666]">{t.dashboardHistory.empty}</p>
          <p className="mt-1 text-[13px] text-[#A5A5A5]">
            {t.dashboardHistory.emptyHint}
          </p>
        </div>
      ) : (
